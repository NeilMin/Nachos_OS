...
public class KThread {
	...
	public static void finish() {
		...
		KThread nextReadyThread = null;
		// if (waitOnReadyQueue.size() != 0) {
		// nextReadyThread = waitOnReadyQueue.poll();
		// nextReadyThread.ready();
		// }
		if (currentThread.waitThread != null) {
			currentThread.waitThread.ready();
		}
	 ...
	 */
	public void join() {
		// this: thread B, currentThread: thread A
		...
		Lib.assertTrue(this.joined == 0);
		this.joined = 1;
		
		boolean status = Machine.interrupt().disable();

		// B is finished do not need to block A
		if (this.status != statusFinished) {
			// check multiple join
			

			// C
			// Lib.assertTrue(!waitOnReadyQueue.contains(currentThread));
			// waitOnReadyQueue.add(currentThread);
		...
		// TODO: could currentThread be different from that from line 289?
	}
	...
	public KThread waitThread = null;

	// public static Queue<KThread> waitOnReadyQueue = new LinkedList<KThread>();
}